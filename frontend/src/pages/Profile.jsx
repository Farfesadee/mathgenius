...
export default function Profile() {
  ...
  // Study goals (stored in user_profiles via tracking API)
  ...
  const handleSave = async () => {
    ...
    // Also save goals to user_profiles via tracking API
    try {
    ...
    <div className="max-w-3xl mx-auto px-6 py-10">
      ...
      <div className="card bg-white p-6 mb-6 flex flex-wrap items-center gap-5">
        <div className={`${avatarBg} w-20 h-20 rounded-2xl flex items-center
                         justify-center font-serif font-black text-white text-3xl
                         shrink-0`}>
          {initials}
        </div>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-[var(--color-border)]
                     text-sm font-semibold text-[var(--color-ink)] hover:border-[var(--color-teal)]
                     hover:text-[var(--color-teal)] transition-all bg-white">
          👀 Share Profile (Parent / Teacher)
        </button>
        <a href="/challenge"
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-[var(--color-border)]
                     text-sm font-semibold text-[var(--color-ink)] hover:border-purple-500
                     hover:text-purple-600 transition-all bg-white">
          ⚔️ Challenge a Friend
        </a>
        <a href="/certificate"
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-[var(--color-border)]
                     text-sm font-semibold text-[var(--color-ink)] hover:border-[var(--color-gold)]
      ...
      {tab === 'referral' && (
        <div className="space-y-5">
          <div className="card bg-white p-6">
            ...
            <div className="mt-4 bg-[#e8f4f4] border border-[var(--color-teal)]
                            rounded-xl p-4">
                ...
                <div className="space-y-2">
                  {referralStats.referrals.map(r => (
                    <div key={r.id}
                      ...
                      <span className="ml-auto text-xs font-mono text-[var(--color-gold)]
                                       font-bold">
                        +100 XP
                      </span>
                    </div>
          ...
          {!profile?.referred_by && (
            <div className="card bg-white p-6">
              ...
              {refMsg && (
                <p className={`mt-2 text-sm ${refMsg.type === 'success' ? 'text-green-600' : 'text-red-500'
                  }`}>
                  {refMsg.text}
                </p>
              )}
            ...
            {BADGES.map(badge => {
              ...
              return (
                <div key={badge.id}
                  ...
                  {earned && (
                    <span className="ml-auto text-[var(--color-gold)] text-lg">✓</span>
                  )}
                </div>
              )
            })}
        ...
        <div className="card bg-white p-6 space-y-4">
          ...
          <div className="flex items-center justify-between py-3 border-b
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => { navigator.clipboard.writeText(refCode); setCopied(true); setTimeout(() => setCopied(false), 2000) }}

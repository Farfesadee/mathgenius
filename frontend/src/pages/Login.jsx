      ...
      <div className="relative">
        <input
          ...
          className="w-full bg-[var(--color-paper)] border-2 border-[var(--color-border)]
                     focus:border-[var(--color-teal)] rounded-xl px-4 py-3 pr-12 text-sm
...
export default function Login({ defaultTab = 'login' }) {
  ...
  const handleGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/teach` }
    })
    if (error) {
      setError(error.message)
      setLoading(false)
          ...
          <div className="bg-white p-8 space-y-5">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 border-2
                         ...
                         rounded-xl py-3 font-semibold text-sm transition-all
                         disabled:opacity-50 bg-white hover:bg-[var(--color-cream)]"
            >
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                ...
                {success && (
                  <div className="bg-green-50 border border-green-200 rounded-xl
                                  px-4 py-3 text-green-700 text-sm">
                    ✅ {success}
                  </div>
                )}
